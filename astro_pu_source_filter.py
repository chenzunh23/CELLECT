...
def classify_sources(table: Table, args: argparse.Namespace) -> dict[str, object]:
    ...
    removed_b |= a_candidate & flag_removed
    reasons: List[List[str]] = [[] for _ in range(len(table))]
    ...
    close_indices = np.flatnonzero(a_candidate & ~removed_b)
    ...
    containment_indices = np.flatnonzero(a_candidate & ~removed_b)
    ...
    b_class = a_candidate & removed_b
    ab_class = a_candidate & ~removed_b
...
def write_outputs(table: Table, result: dict[str, object], args: argparse.Namespace) -> List[Path]:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=("row", "id", "x", "y", "mag", "class", "reason", "kron_a", "kron_b", "kron_area"),
        )
        writer.writeheader()
