...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        ...
        self.conv3d_c2 = nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter, self.base_n_filter * 2, kernel_size=3, stride=1, padding=1,
                                   bias=False))
        self.conv3d_c2s1=nn.Conv3d(self.base_n_filter*2, self.base_n_filter * 2, kernel_size=3, stride=1, padding=1,
                                   bias=False)
        self.conv3d_c2s2=nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter, self.base_n_filter * 2, kernel_size=1, stride=1, padding=0,
                                   bias=False))
        self.norm_lrelu_conv_c2 = self.norm_lrelu_conv(self.base_n_filter * 2, self.base_n_filter * 2)
        self.inorm3d_c2 = nn.InstanceNorm3d(self.base_n_filter * 2)

        self.conv3d_c3 =nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 4, kernel_size=3, stride=1, padding=1,
                                   bias=False))
        self.conv3d_c3s1=nn.Conv3d(self.base_n_filter*4, self.base_n_filter * 4, kernel_size=3, stride=1, padding=1,
                                   bias=False)
        self.conv3d_c3s2=nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter*2, self.base_n_filter * 4, kernel_size=1, stride=1, padding=0,
                                   bias=False))
        self.norm_lrelu_conv_c3 = self.norm_lrelu_conv(self.base_n_filter * 4, self.base_n_filter * 4)
        self.inorm3d_c3 = nn.InstanceNorm3d(self.base_n_filter * 4)

        self.conv3d_c4 = nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 8, kernel_size=3, stride=1, padding=1,
                                   bias=False))
        self.conv3d_c4s1=nn.Conv3d(self.base_n_filter*8, self.base_n_filter * 8, kernel_size=3, stride=1, padding=1,
                                   bias=False)
        self.conv3d_c4s2=nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter*4, self.base_n_filter * 8, kernel_size=1, stride=1, padding=0,
                                   bias=False))
        self.norm_lrelu_conv_c4 = self.norm_lrelu_conv(self.base_n_filter * 8, self.base_n_filter * 8)
        self.inorm3d_c4 = nn.InstanceNorm3d(self.base_n_filter * 8)

        self.conv3d_c5 =nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 16, kernel_size=3, stride=1, padding=1,
                                   bias=False))
        self.conv3d_c5s1=nn.Conv3d(self.base_n_filter*16, self.base_n_filter * 16, kernel_size=3, stride=1, padding=1,
                                   bias=False)
        self.conv3d_c5s2=nn.Sequential(nn.MaxPool3d((2,2,1)),nn.Conv3d(self.base_n_filter*8, self.base_n_filter * 16, kernel_size=1, stride=1, padding=0,
                                   bias=False))
